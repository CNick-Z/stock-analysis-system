...
| 任务 | 状态 | 说明 |
|------|------|------|
| direction 逻辑修复 | ✅ | `_get_trend_direction()` 基于波浪状态 |
| CZSC 初始化修复 | ✅ | 延迟初始化策略 |
| PositionManager 实现 | ✅ | `entry_long/verify_position/get_exit_action` |
| 止损变量 BUG 修复 | ✅ | `daily_czsc=None` 检查 |

**新增类**：`WaveEngine`（双周期引擎）、`PositionManager`（持仓管理器）
