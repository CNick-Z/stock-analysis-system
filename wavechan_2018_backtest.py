#!/usr/bin/env python3
"""
Wavechan 策略 2018年 基线测试（熊市最惨的一年）
用修正后的严格逻辑（次日开盘价执行）
"""
import sys, os, time
...
def run_backtest(year_df, dates, config):
    ...
    for date in dates:
        ...
        for sym in list(positions.keys()):
            ...
            if reason:
                sell_price = get_next_open(date, sym)
        ...
        if len(positions) < config['max_positions']:
            ...
            for _, row in cands.iterrows():
                ...
                trades.append({
                    ...
                    'buy_price': buy_price, 'wave_stage': row.get('wave_stage', ''),
                })
...
if __name__ == '__main__':
    print("=" * 60)
    print("Wavechan 2018年 基线测试（熊市惨烈年）")
    print("=" * 60)
    ...
    print(f"数据: {len(df)}条, {df['symbol'].nunique()}只, 加载{time.time()-t0:.1f}s")

    t1 = time.time()
    features = compute_wavechan_features_fast(df, config)
    print(f"特征: {time.time()-t1:.1f}s")

    year_df = features[features['date'].str.startswith('2018-')].copy()
    dates = sorted(year_df['date'].unique())
    print(f"2018: {dates[0]}~{dates[-1]}, {len(dates)}交易日\n")
    ...
    # 对比沪深300 2018年跌幅
    print(f"\n📌 参考: 沪深300 2018年跌幅约 -25%")
    print(f"📌 对比Score策略 2018年: 暂无数据（Score仅回测2020+）")
    ...
    for e in result['equity']:
        month = e['date'][:7]
        monthly[month] = e['total']
    for m, v in sorted(monthly.items()):
        cumret = (v - CONFIG['initial_cash']) / CONFIG['initial_cash']
        print(f"  {m}: {v:>12,.0f} ({cumret:+.2%})")

    # 打印最大亏损月份
    print(f"\n--- 最差5笔卖出 ---")
    for t in sorted(sells, key=lambda x: x['ret'])[:5]:
        emoji = "❌"
        print(f"  {emoji} {t['date']} {t['symbol']} 买{t['buy_price']}→卖{t['sell_price']} {t['ret']:+.2%} 原因:{t['reason']}")
