...
【统一框架】
  - 策略逻辑: strategies.score.v8.strategy.ScoreV8Strategy（与回测共用）
  - 数据加载: utils.data_loader.load_strategy_data（含 money_flow_trend 计算）
  - 两者完全对齐，模拟盘 ≈ 回测环境
...
from strategies.score.v8.strategy import (
    ScoreV8Strategy,
...
def load_latest_data():
    """
    加载2026年完整策略数据（含资金流）
    统一使用 data_loader.load_strategy_data，与回测完全一致
    """
    # 使用统一数据加载（包含 money_flow_trend 计算）
    df = load_strategy_data(years=[2026], add_money_flow=True)

    # 处理换手率重名字段（合并后可能产生 _x/_y 后缀）
    for col in ('turnover_rate', 'vol_ratio'):
        if f'{col}_y' in df.columns:
            df[col] = df[f'{col}_y']
        elif f'{col}_x' in df.columns:
            df[col] = df[f'{col}_x']

    # 计算选股条件（使用统一策略模块，与回测完全一致）
    df = compute_conditions(df)

    return df


def run_day(state, daily, date, dates, i, n_dates, _strategy):
    """每日交易逻辑"""
    p = state['portfolio']
    ...
    if next_day and len(p['positions']) < TOP_N:
        ...
        for _, row in candidates.head(slots).iterrows():
            ...
            p['positions'][sym] = {
                ...
                'buy_date': date,
                'buy_price': next_open,
                # _exit_consecutive_days 由策略 on_tick() 内部管理，框架无需初始化
            }
            p['trades'].append({
...
def main():
    ...
    # 策略实例（整个回测周期复用，策略内部维护计数器）
    _strategy = ScoreV8Strategy()

    for i in range(start_idx, n_dates):
        date = dates[i]
        daily = df[df['date'] == date].copy()
        run_day(state, daily, date, dates, i, n_dates, _strategy)
