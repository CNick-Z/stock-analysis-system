...
class BaseFramework:
    ...
    def _process_sells(self, daily: pd.DataFrame, market: dict):
        ...
        for sym in list(self.positions.keys()):
            ...
            # 卖出价：当日收盘（更真实反映信号触发时的市场价格）
            exec_price = r.get("close")
    ...
    def _process_buys(self, full_df: pd.DataFrame, daily: pd.DataFrame, market: dict):
        ...
        # ---- 第一遍：构建所有候选信号记录 ----
        today_candidates = []
        # 同时拉取之前未执行的 pending 候选，在本次交易日给予执行机会
        pending_old = [c for c in self.candidates if c.get("status") == "pending"]
        for cand in pending_old:
            cand["is_prev_pending"] = True
            cand["status"] = "pending"  # 重置状态，每天重新评估
        ...
        # ---- 第二遍：执行昨日pending信号 ----
        # 信号只保留 signal + date，执行时才处理所有状态
        to_buy = []
        buy_count = 0
        for cand in today_candidates:
            if cand.get("status") != "pending":
                continue
            if not cand.get("is_prev_pending"):
                # 今天新候选：只记录，不执行
                continue
            sym = cand["symbol"]
            day_row = daily[daily["symbol"] == sym]
            ...
            if pd.isna(exec_price) or exec_price <= 0:
                exec_price = r.get("close", 0) or 0
            exec_price = exec_price * (1 + self.slippage_pct)
            if self.cash < exec_price * 100:
                cand["status"] = "X5 - 资金不足"
                cand["reason"] = "可用资金不足"
                continue
        ...
        if not to_buy:
            # 保留 pending 旧候选 + 新候选（今日生成但未执行）
            kept_pending = [c for c in self.candidates if c.get("status") == "pending"]
            self.candidates = kept_pending + today_candidates
            return
        ...
        # ---- 合并候选 ----
        # 保留所有 pending 的旧候选（还没执行过）；非 pending 的旧候选（已买入/已满仓）清理掉
        kept_pending = [c for c in self.candidates if c.get("status") == "pending"]
        self.candidates = kept_pending + today_candidates
