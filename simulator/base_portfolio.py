...
class BasePortfolio:
    ...
    def _process_exits(
        ...
        for sym, pos in list(self.positions.items()):
            ...
            # 策略内部更新出场计数器（框架无需感知计数器存在）
            if hasattr(self.strategy, 'on_tick'):
                self.strategy.on_tick(row, pos)

            # ========== 改动5: 分批建仓 - 执行加仓 ==========
            # 条件：awaiting_add=True + 有确认信号 + 现金充足
            if pos.get('awaiting_add', False) and self.cash > 0:
                buy_price_add = row.get('next_open') or row.get('close')
                if buy_price_add and buy_price_add > 0:
                    # 计算剩余目标仓位
                    target_pct = pos.get('target_pct', 0.02)
                    first_qty = pos.get('first_buy_qty', 0)
                    first_price = pos.get('first_buy_price', 0)
                    if first_qty > 0 and first_price > 0:
                        # 目标股数 vs 当前股数
                        row_dict = row.to_dict() if hasattr(row, 'to_dict') else dict(row)
                        add_confirmed = (
                            row_dict.get('signal_type', '') in ('W4_BUY', 'W2_BUY', 'C_BUY') and
                            row_dict.get('signal_status', '') in ('CONFIRMED', 'ALERT')
                        )
                        if add_confirmed:
                            # 首仓已开（qty >= first_qty），且有确认信号 → 加仓
                            target_qty = int((target_pct * self.cash) / buy_price_add)
                            target_qty = (target_qty // 100) * 100
                            add_qty = target_qty - pos['qty']
                            if add_qty >= 100 and add_qty * buy_price_add <= self.cash:
                                self.cash -= add_qty * buy_price_add
                                pos['qty'] += add_qty
                                # 更新 avg_cost
                                total_cost = first_qty * first_price + add_qty * buy_price_add
                                pos['avg_cost'] = round(total_cost / pos['qty'], 2)
                                pos['awaiting_add'] = False
                                logger.debug(
                                    f"[{self.name}] 加仓 {sym} x{add_qty} @{buy_price_add:.2f} "
                                    f"(目标仓位{target_pct:.0%}, 现金余额: {self.cash:.2f})"
                                )
    ...
    def _process_buys(
        ...
        for _, row in top_n.iterrows():
            ...
            # ========== 改动4: 仓位分级（根据信号置信度调整仓位） ==========
            entry_signal = row.get('signal_type', 'W2_BUY')
            confidence = row.get('total_score', 0)

            # 仓位分级
            if confidence >= 70:
                position_pct = 0.05   # S级：5%仓位
            elif confidence >= 60:
                position_pct = 0.03   # A级：3%仓位
            else:
                position_pct = 0.02   # B级：2%仓位

            # C_BUY（一买）特殊：不超过2%
            if entry_signal == 'C_BUY':
                position_pct = min(position_pct, 0.02)

            # 使用分级后的仓位计算实际买入金额
            adjusted_cash = available_cash * position_pct / max(len(top_n), 1)

            qty = int(adjusted_cash / buy_price)
            ...
            trade = {
                ...
                'rsi': round(float(row.get('rsi_14', 0) or 0), 1),
                'reason': f"SCORE={row.get('score', 0):.4f}|CONFIDENCE={confidence:.0f}|POS={position_pct:.0%}",
                'cost': round(cost, 2),
                'entry_sma20_le_sma55': entry_sma20_le_sma55,
            }
            buy_trades.append(trade)
            self._open_position(sym, row, qty, buy_price, date, entry_sma20_le_sma55, position_pct)
    ...
    def _open_position(
        ...
        Args:
            ...
            position_pct: 目标仓位比例（用于分批建仓）
        """
        cost = qty * price
        self.cash -= cost

        # ========== 改动5: 分批建仓标记 ==========
        # 记录加仓标记：首仓完成后，等待确认信号加满到 target_pct
        self.positions[symbol] = {
            ...
            # 分批建仓追踪
            'awaiting_add': True,          # 等待确认后加仓
            'target_pct': position_pct,    # 目标仓位
            'first_buy_price': price,      # 首仓价格（用于判断是否确认）
            'first_buy_qty': qty,          # 首仓数量
            # 持仓期间最高价追踪（用于移动止盈）
