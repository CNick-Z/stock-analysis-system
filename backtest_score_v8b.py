...
def load_year(year):
    ...
    # 方案B新增: 前一日CCI（用于CCI死叉判断）
    df['cci_prev'] = df.groupby('symbol')['cci_20'].shift(1)
    # 当日涨幅（用于追踪连续下跌）
...
def run_backtest_year(year):
    ...
    for i, date in enumerate(dates):
        ...
        for sym, pos in list(portfolio['positions'].items()):
            ...
            prev_close = pos.get('prev_close', today_close)
            if today_close < prev_close:
                pos['consecutive_down_days'] = pos.get('consecutive_down_days', 0) + 1
            else:
                pos['consecutive_down_days'] = 0
            pos['prev_close'] = today_close
            
            # 止损 7%（放宽，让利润奔跑）
            if next_open < pos['avg_cost'] * 0.93:
                should_sell = True
                reason = "stop_loss_7pct"
            # 【方案B核心】让利润奔跑，不用15%止盈，改用趋势跟踪出场
            # 条件1: MA20死叉（MA20下穿MA55）
            elif row['sma_20'] > row['sma_55']:
                should_sell = True
                reason = "ma20_death_cross"
            # 条件2: CCI从超买区回落（CCI死叉信号）
            elif (row['cci_20'] > 100) and (row['cci_20'] < row.get('cci_prev', row['cci_20'])):
                should_sell = True
                reason = "cci_death_cross"
            # 条件3: WR极度超买（WR > -20）
            elif row['williams_r'] > -20:
                should_sell = True
                reason = "wr_overbought"
            # 条件4: 连续3日收盘价下跌
            elif pos.get('consecutive_down_days', 0) >= 3:
                should_sell = True
                reason = "consecutive_3down"
        ...
        if next_day and len(portfolio['positions']) < TOP_N:
            ...
            for _, row in candidates.head(slots).iterrows():
                ...
                portfolio['positions'][sym] = {
                    ...
                    'consecutive_down_days': 0  # 方案B:追踪连续下跌天数
                }
                portfolio['trades'].append({
