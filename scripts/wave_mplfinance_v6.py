...
def identify_extrema(bi_df: pd.DataFrame, min_amplitude_pct: float = 0.0) -> pd.DataFrame:
    """识别所有有效的局部极值点"""
    ...
    for rev in reversals:
        ...
        if rev['type'] == prev['type']:
            # 同类型：保留波动更大的（更高的高点或更低的低点）
            if rev['type'] == 'high' and rev['price'] > prev['price']:
                filtered[-1] = rev  # 更高的低点
            elif rev['type'] == 'low' and rev['price'] < prev['price']:
                filtered[-1] = rev  # 更低的低点
            # 否则保留原来的
...
def draw_wave_chart_mplfinance(
    ...
    # ===== 准备波浪线的 addplot 数据 =====
    # 关键修复：用线性插值连接相邻极值点，使波浪线连续
    # 相邻段在极值点处价格相同，自然连成一条波浪线
    wave_plot_data = {}
    wave_colors = {}
    wave_labels = {}
    ...
    for i, wave in enumerate(waves):
        ...
        # 【核心修复】用线性插值填满整个波段
        # 极值日期可能是非交易日（周末/节假日），用 method='nearest' 找到 OHLC 中对应的实际交易日
        wave_series = pd.Series(index=ohlc_df.index, dtype=float)
        
        # 用最近邻找到 OHLC index 中对应的实际交易日
        sd_idx = ohlc_df.index.get_indexer([wave['start_date']], method='nearest')[0]
        ed_idx = ohlc_df.index.get_indexer([wave['end_date']], method='nearest')[0]
        act_start_date = ohlc_df.index[sd_idx]
        act_end_date = ohlc_df.index[ed_idx]
        
        # 取这段实际交易日区间
        seg_mask = (ohlc_df.index >= act_start_date) & (ohlc_df.index <= act_end_date)
        seg_dates = ohlc_df.index[seg_mask]
        n = len(seg_dates)
        
        if n >= 2:
            interpolated = np.linspace(wave['start_price'], wave['end_price'], n)
            wave_series.loc[seg_dates] = interpolated
        elif n == 1:
            wave_series.loc[act_start_date] = wave['start_price']
    ...
    for _, ext in extrema_df.iterrows():
        date = ext['date']
        price = ext['price']
        # 极值日期可能是非交易日，用最近邻映射到实际交易日
        idx = ohlc_df.index.get_indexer([date], method='nearest')[0]
        act_date = ohlc_df.index[idx]
        if ext['type'] == 'high':
            extrema_high_series[act_date] = price
        else:
            extrema_low_series[act_date] = price
    
    # 波浪转折点用【双层圆形】标记：外圈有色边框 + 内圈白色填充
    # 先画外圈（大一点，有色）
    add_plots.append(mpf.make_addplot(extrema_high_series, type='scatter', marker='o',
                                       color='#C0392B', markersize=130, alpha=0.95))
    # 再画内圈（小一点，白色）
    add_plots.append(mpf.make_addplot(extrema_high_series, type='scatter', marker='o',
                                       color='#ffffff', markersize=70, alpha=0.95))
    # 低点外圈
    add_plots.append(mpf.make_addplot(extrema_low_series, type='scatter', marker='o',
                                       color='#1E8449', markersize=130, alpha=0.95))
    # 低点内圈
    add_plots.append(mpf.make_addplot(extrema_low_series, type='scatter', marker='o',
                                       color='#ffffff', markersize=70, alpha=0.95))
    ...
    for i, wave in enumerate(waves):
        # 把极值日期映射到实际交易日（避免周末/节假日导致 x 轴对不齐）
        sd_idx = ohlc_df.index.get_indexer([wave['start_date']], method='nearest')[0]
        ed_idx = ohlc_df.index.get_indexer([wave['end_date']], method='nearest')[0]
        act_start = ohlc_df.index[sd_idx]
        act_end = ohlc_df.index[ed_idx]
        mid_dt = act_start + (act_end - act_start) / 2
    ...
    for _, ext in extrema_df.iterrows():
        ...
        ax.annotate(
            f'{price:.2f}',
            xy=(act_date, price),
            xytext=(0, 12 if ext['type'] == 'low' else -18),
            textcoords='offset points',
            ha='center',
            fontsize=9,
            color=fontcolor,
            fontweight='bold',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='#1a1a2e', edgecolor='none', alpha=0.8),
        )
    ...
    ax.text(0.5, 1.02, legend_text, transform=ax.transAxes,
            ha='center', fontsize=11, color='#dddddd',
            bbox=dict(boxstyle='round,pad=0.4', facecolor='#1a1a2e', edgecolor='none', alpha=0.8))
    
    # 延续区间标注
    if continuation_end_idx > 1:
        cont_start = extrema_df.iloc[0]['date']
        cont_end = extrema_df.iloc[continuation_end_idx - 1]['date']
        cs_idx = ohlc_df.index.get_indexer([cont_start], method='nearest')[0]
        ce_idx = ohlc_df.index.get_indexer([cont_end], method='nearest')[0]
        act_cont_start = ohlc_df.index[cs_idx]
        act_cont_end = ohlc_df.index[ce_idx]
        mid_cont = act_cont_start + (act_cont_end - act_cont_start) / 2
...
def main():
    ...
    q4_extrema = identify_extrema(q4_bi, min_amplitude_pct=0.0)
    year_extrema = identify_extrema(year_bi, min_amplitude_pct=0.0)
